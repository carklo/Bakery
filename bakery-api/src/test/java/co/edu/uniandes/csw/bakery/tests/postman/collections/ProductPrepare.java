...
public class ProductPrepare  {
   ...
   private static final String PATH = System.getProperty("user.dir").concat("\\collections\\postman_collectionProduct.json");
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,ProductDetailDTO products,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(products);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(ProductDetailDTO products) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:products.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(products, null).toString().concat("\",\""))); 
           }
