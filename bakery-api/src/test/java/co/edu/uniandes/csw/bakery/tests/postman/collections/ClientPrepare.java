 ...
 * @author Asistente
 */
public class ClientPrepare  {
    
   private static final String PATH = System.getProperty("user.dir").concat("\\collections\\postman_collectionClient.json");
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,ClientDetailDTO clients,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(clients);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(ClientDetailDTO clients) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:clients.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(clients, null).toString().concat("\",\""))); 
           }
