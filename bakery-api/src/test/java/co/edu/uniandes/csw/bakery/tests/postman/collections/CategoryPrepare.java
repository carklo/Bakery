...
public class CategoryPrepare  {
   ...
   private static final String PATH = System.getProperty("user.dir").concat("\\collections\\postman_collectionCategory.json");
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,CategoryDetailDTO categorys,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(categorys);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(CategoryDetailDTO categorys) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:categorys.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(categorys, null).toString().concat("\",\""))); 
           }
